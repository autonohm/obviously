...
public:
	 ...
	 */
	RansacMatching(unsigned int trials = 50, double epsThresh = 0.15,
			unsigned int sizeControlSet = 180, bool clipPeripheralArea = false);
	...
	obvious::Matrix match(obvious::Matrix* M, obvious::Matrix* S,
			double phiMax = M_PI / 4.0, double resolution = 0.0);
